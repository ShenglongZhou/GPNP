function data = CSdata(problem,m,n,s,nf)
% Inputs:
%       problem   -- can be 'CS','QCS'
...
%       data.xtrue       --  n x 1 order 'true' sparse solution, (optional) 
%
% which satisfies       b = A*xopt + nf*noise for CS
%                       b = (A*xopt).^2 + nf*noise for QCS
if nargin  < 4; disp(' Error!!! Inputs are not enough!!!'); return; end
if nargin  < 5; nf = 0; end
disp(' Please wait for data generation ')
...
switch problem
    case 'CS' 
       ...
       Ax         = data.A(:,Tx)*x(Tx);
       data.b     = Ax + nf*randn(m,1);  
       data.obj   = norm(data.b-Ax)^2;                         
    case  'QCS'
       Ax         = data.A(:,Tx)*x(Tx);
       data.b     = Ax.^2 + nf*randn(m,1);
       data.obj   = norm(data.b-Ax.^2)^2/4/m; 
end
disp(' Done for data generation ');
disp(' --------------------------------------');
end
